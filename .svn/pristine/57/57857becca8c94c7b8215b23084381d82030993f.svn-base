 ...
 *
 * @file DES3加密服务
 * @description 传输内容加密类，其中 key = ConstantService.ENC_KEY,
 *                                 des3iv = ConstantService.ENC_IV
 * =========================================================================
 *
 ...
 * =========================================================================
 *
 * @example EncryptService.encrypt('javascript') //Vuf2CnZd3DA11DBu20lVNg==
 * @example EncryptService.decrypt('Vuf2CnZd3DA11DBu20lVNg==') //javascript
 *
 * @author linzixiong
 * @date 2016/12/14
 * @version 1.0
...
export default class EncryptService {
    ...
    static key = ConstantService.ENC_KEY;
    static des3iv = ConstantService.ENC_IV;
    ...
    static encrypt(input) {
        let genKey = _genkey(this.key, 0, 24);
        return base64.encode(_des(genKey.key, input, 1, 1, this.des3iv, 1));
    }
    ...
    static decrypt(input) {
        let genKey = _genkey(this.key, 0, 24);
        return _des(genKey.key, base64.decode(input), 0, 1, this.des3iv, 1);
    }

}

let _des = function (key, message, encrypt, mode, iv, padding) {
...
let _genkey = function (key, start, end) {
    //8 byte / 64 bit Key (DES) or 192 bit Key
    return {key: _pad(key.slice(start, end)), vector: 1};
};


let _pad = function (key) {
